...
function scrapeDirectoryToJson(
  dir: string,
...
function scrapeDirectoryToPlainText(
  dir: string,
  ignorePatterns: string[] = [],
  prefix: string = '',
): string {
  ...
  files.forEach((file) => {
    ...
    if (stat.isDirectory()) {
      ...
      result += scrapeDirectoryToPlainText(
        filePath,
        ignorePatterns,
...
export async function scrapeRepositoryToJson(
  repoUrl: string,
  ignorePatterns: string[] = [],
): Promise<FileData[]> {
...
export async function scrapeRepositoryToPlainText(
  repoUrl: string,
  ignorePatterns: string[] = [],
): Promise<string> {
  const repoName = repoUrl.split('/').pop()?.replace('.git', '');
